# Streaming readiness scoring example using Julia standard libraries.
...
checks = read_csv_simple("data/governance_checks.csv")
status_weight = Dict("pass" => 1.0, "approved" => 1.0, "in_review" => 0.7, "watch" => 0.55, "warn" => 0.4, "needs_revision" => 0.15)
...
mkpath("outputs")
open("outputs/streaming_readiness_julia.csv", "w") do io
    println(io, "governance_check_count,mean_governance_score")
    println(io, "$(length(scores)),$(readiness)")
end

open("outputs/julia_manifest.txt", "w") do io
    println(io, "workflow=streaming-readiness-score")
...
println("Julia streaming readiness scoring complete")
