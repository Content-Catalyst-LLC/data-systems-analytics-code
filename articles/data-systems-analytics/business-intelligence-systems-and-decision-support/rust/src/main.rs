...
fn main() -> std::io::Result<()> {
    ...
    let status_idx = headers.iter().position(|x| *x == "semantic_status").unwrap();
    ...
    println!("Rust metric ownership inventory:");
    for (owner, count) in owner_counts {
        println!("owner:{}={}", owner, count);
    }

    println!("Rust semantic status inventory:");
